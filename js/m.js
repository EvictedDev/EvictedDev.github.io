...
for (let i = 0; i < pricing.length; i++) {
  ...
  let newPrice = priceValue - priceValue * 0.1;

  if (i === pricing.length - 1) {
    // Apply a 0.5 discount to the last element's newPrice
    newPrice = priceValue - priceValue * 0.2;
  }
  ...
  pricing[i].innerHTML =
    `<span style="text-decoration: line-through;margin-right: 1vw;">${priceValue}$</span>` +
    newPrice +
    "$";
...
$(function () {
  ...
  $("#products .products").on(
    ...
    function () {
      if (
        $(this).closest("#products").hasClass("cheats-status") ||
        $(this).hasClass("non-cheats")
      )
        return false;
      ...
      timerProductHideTime = setTimeout(function () {
        if (productHoverHideTime == 1) {
          $("#products .products .product-wrapper .product").each(function () {
            if (!$(this).hasClass("non-cheats")) $(this).addClass("hide-time");
          });
  ...
  $("#products .products").on(
    ...
    function () {
      ...
      $("#products .products .product-wrapper .product").removeClass(
        "hide-time"
      );
    }
  );
  ...
  $(
    "#products .products .product-wrapper .product.non-cheats .price"
  ).removeClass("change-price-mb-dollar");
  $("#products .products .product-wrapper .product.non-cheats .price").text(
    "Пусто"
  );
  ...
  if (
    $(
      '#product .main-info .left-block .carousel .screens .item[data-id="0"]'
    ).find("img").length > 0
  ) {
    $("#product .main-info .left-block .carousel .main-screen img").attr(
      "src",
      $(
        '#product .main-info .left-block .carousel .screens .item[data-id="0"] img'
      ).attr("src")
    );
  } else {
    $("#product .main-info .left-block .carousel .main-screen img").hide();
    $("#product .main-info .left-block .carousel .main-screen iframe").show();
    $("#product .main-info .left-block .carousel .main-screen iframe").attr(
      "src",
      $(this).find(".video-figure").data("src")
    );
  }
  $(
    '#product .main-info .left-block .carousel .screens .item[data-id="0"]'
  ...
  let carouselProductPageTimer = null,
    ...
    hovCarouseloff = 0,
    countScreens = $(
      "#product .main-info .left-block .carousel .screens .item"
    ).length;

  carouselProductPage(
    "#product .main-info .left-block .carousel .screens .item.active"
  );
  ...
  function carouselProductPage(el) {
    ...
    let widthNow = $(
        "#product .main-info .left-block .carousel .main-screen span"
      ).width(),
      id = $(el).data("id"),
      ...
      carouselProductPageTimer = setInterval(function () {
        if (timerProductPageOff == 0) {
          ...
          if ($(block + " span").width() > 435) {
            ...
            $(
              '#product .main-info .left-block .carousel .screens .item[data-id="' +
                id +
                '"]'
            ).removeClass("active");
            ...
            if (
              ...
              $(
                "#product .main-info .left-block .carousel .main-screen .count b"
              ).text("1");
            }
            ...
            if (
              $(
                "#product .main-info .left-block .carousel .screens .item.active"
              ).find(".video-figure").length == 0
            ) {
              $(block + " img").show();
              $(block + " img").attr(
                "src",
                $(
                  "#product .main-info .left-block .carousel .screens .item.active img"
                ).attr("src")
              );
            } else {
              ...
              $(block + " iframe").attr(
                "src",
                $(
                  "#product .main-info .left-block .carousel .screens .item.active .video-figure"
                ).data("src")
              );
            }
  ...
  $("#product .main-info .left-block .carousel .screens .item").click(
    function () {
      stopInterval(carouselProductPageTimer);
      $("#product .main-info .left-block .carousel .screens .item").removeClass(
        "active"
      );
      $("#product .main-info .left-block .carousel .main-screen span").css(
        "width",
        "0%"
      );
      $("#product .main-info .left-block .carousel .main-screen .count b").text(
        $(this).data("id") + 1
      );
      ...
      if ($(this).find(".video-figure").length == 0) {
        ...
        $(
          "#product .main-info .left-block .carousel .main-screen iframe"
        ).show();
  ...
  $("#product .main-info .left-block .carousel .main-screen").click(
    function () {
      stopInterval(carouselProductPageTimer);
      $("#product .main-info .left-block .carousel .main-screen span").css(
        "width",
      ...
      $("#product .main-info .left-block .carousel .preview img").attr(
        "src",
        $("#product .main-info .left-block .carousel .main-screen img").attr(
          "src"
        )
  ...
  $("#product .main-info .left-block .carousel .preview").click(function () {
    if (hovCarouseloff == 0) {
      $("#product .main-info .left-block .carousel .preview").removeClass(
        "show"
      );
      carouselProductPage(
        "#product .main-info .left-block .carousel .screens .item.active"
  ...
  $("#product .main-info .left-block .carousel .preview .close").click(
    function () {
      $("#product .main-info .left-block .carousel .preview").removeClass(
        "show"
      );
  ...
  $("#product .main-info .left-block .carousel .main-screen").hover(
    function () {
  ...
  $("#product .main-info .left-block .carousel .preview .container").hover(
    function () {
      hovCarouseloff = 1;
    },
    function () {
  ...
  $("#products .sort-games .sort-by-cheats .list .sort").click(function () {
    ...
    $("#products .sort-games .sort-by-cheats .list .sort").removeClass(
      "active"
    );
    $(this).addClass("active");
    ...
    if (thisSort != "*") {
      $("#products .products .product-wrapper").each(function () {
        ...
        if (cheats.indexOf(thisSort) >= 0) $(this).show();
      });
    } else {
  ...
  $("#products .sort-games .sort-by-games-filters .list .sort").click(
    function () {
      let selectedSort = $(
          "#products .sort-games .sort-by-cheats .list .sort.active"
        ).data("filter"),
        selectedStatus = $(this).data("sort");

      $("#products .sort-games .sort-by-games-filters .list .sort").removeClass(
        "active"
      );
      $(this).addClass("active");

      if (selectedStatus == "popular") {
        $('#products .products .product-wrapper[data-popular="*"]').each(
          function () {
            $(this)
              .closest(".product-wrapper")
              .remove()
              .appendTo("#products .products");
          }
        );
      } else {
        $('#products .products .product-wrapper[data-popular="popular"]').each(
          function () {
            $(this)
              .closest(".product-wrapper")
              .remove()
      ...
      $("#products .products .product-wrapper .product.non-cheats").each(
        function () {
          $(this)
            .closest(".product-wrapper")
            .remove()
  ...
  $("#product .main-info .right-block .select .select-sub .sel").click(
    function () {
      $(
        "#product .main-info .right-block .select .select-sub .sel"
      ).removeClass("active");
      $(this).addClass("active");

      $("#product .main-info .right-block .go-buy .btn.goih .price-in").text(
      ...
      if ($.cookie("lang") != null) {
        if ($.cookie("lang") == "en") {
          $(
            "#product .main-info .right-block .go-buy .btn.goih .price-in"
          ).text($(this).find(".price").data("price-dollar") + "$");
        }
      }
    }
  );
  ...
  $(
    "#product .main-info .right-block .select .select-sub .sel-wrapper:first-child .sel"
  ).addClass("active");

  $("#product .main-info .right-block .go-buy .btn.goih .price-in").text(
    $(
      "#product .main-info .right-block .select .select-sub .sel-wrapper .sel.active .price"
    ).data("price") + "₽"
  );
  ...
  $(
    "#product .content .main-info .right-block .go-buy .report .btn.go-ticket-go"
  ).click(function () {
    window.location.href = "tickets.html";
  });

  $("#product .content .main-info .right-block .go-buy .btn.goih").click(
    function () {
  ...
  function showNotification(msg) {
    if ($("body .notification-wrapper-note").length > 0) return false;

    let note =
      '<div class="notification-wrapper-note"><i class="fa-solid fa-triangle-exclamation"></i>' +
      msg +
      "</div>";
    $("body").append(note);
  ...
  $("#products .product .cheats span.cheat").hover(
    function () {
      hovSpanCheatoff = 1;
  ...
  $("#products .search-block .search input").keyup(function () {
    ...
    if (searchVal != "") {
      $("#products .products .product-wrapper").hide();
      $(
        '#products .products .product-wrapper[data-filter*="' +
      ...
      $(
        '#products .sort-games .sort-by-cheats .list .sort[data-filter="*"]'
      ).addClass("active");
      $("#products .sort-games .sort-by-games-filters .list .sort").removeClass(
        "active"
      );
      $(
        '#products .sort-games .sort-by-games-filters .list .sort[data-sort="popular"]'
      ).addClass("active");
    } else {
      $("#products .products .product-wrapper").show();
    }
  ...
  $(".modal-wrapper .modal.send-review .go-send .rate p").click(function () {
    ...
    switch ($(this).data("id")) {
      case 0:
        ...
        break;
      case 1:
        $(block + '[data-id="0"], ' + block + '[data-id="1"]').addClass(
          "active"
        );
  ...
  $(".modal-wrapper .modal.send-review .go-send .btn").click(function () {
    let name = $(
        ".modal-wrapper .modal.send-review .input.nickname input"
      ).val(),
    ...
    if (
      $(".modal-wrapper .modal.send-review .go-send .rate p.active").length == 0
    )
      error = "Оцените продукт";
    if (text.length == 0) error = "Напишите своё мнение о продукте";
    else if (name.length > 15)
      error = 'Поле "Ваше мнение" не более 150 символов';
    if (email.length == 0) error = "Напишите свою эл. почту";
    if (name.length < 4) error = "Напишите своё имя";
    else if (name.length > 15) error = 'Поле "Ваше имя" не более 15 символов';
    ...
    if (error == null) {
      if (checkEmail(email)) {
        ...
        if (existReview) {
          ...
          $(".modal-wrapper .modal.accept-email input.custom").data(
            "cheat-id",
            cheat_id
          );
          $(".modal-wrapper .modal.accept-email input.custom").data(
  ...
  $("#tickets .content.select .block .btn").click(function () {
    ...
    if (checkEmail(email)) {
      if ($(this).closest(".block").hasClass("create-new")) {
        ...
        $(".modal-wrapper .modal.tickets-create-new .btn.go-create").data(
          "email",
          email
        );
      } else {
  ...
  $("#createTicketFilego").on("change", function () {
    ...
    $(".modal-wrapper .modal.tickets-create-new p.uploaded_img").remove();
    $(".modal-wrapper .modal.tickets-create-new .select-img").after(
      '<p class="uploaded_img">' + files_createTicket[0].name + "</p>"
    );
  });
  ...
  $(".modal-wrapper .modal.tickets-create-new .btn.go-create").click(
    function () {
  ...
  $("#tickets .content.show-tickets table tbody td a").click(function () {
    ...
    showModal(
      '.tickets-views .ticket-view[data-id="' + $(this).data("id") + '"]'
    );
    let chat =
      '.modal-wrapper .modal.tickets-views .ticket-view[data-id="' +
      $(this).data("id") +
      '"] .chat';
    $(chat).animate(
      {
  ...
  $(
    "#tickets .content.show-tickets table tbody td .btn.accept-ticket-email"
  ).click(function () {
    let email = $(this).data("email"),
      ticket_id = $(this).data("id");

    $(".modal-wrapper .modal.accept-email .description").html(
      "На ваш электронный адрес <span></span> был отправлен код подтверждения для активации обращения."
    );
  ...
  $("#sendMsgTicketFilego").on("change", function () {
    ...
    if (hasInvalidFiles) {
      $(this).val("");
      $(
        ".modal-wrapper .modal.tickets-views .ticket-view p.uploaded_img"
      ).remove();
      showNotification("Неподдерживаемый формат изобр.");
      return false;
    }
    ...
    $(".modal-wrapper .modal.tickets-views .ticket-view .select-img").after(
      '<p class="uploaded_img">' + files_sendMsgTicket[0].name + "</p>"
    );
  });

  $(".modal-wrapper .modal.tickets-views .ticket-view .select-img").click(
    function () {
      $("#sendMsgTicketFilego")[0].click();
    }
  );

  $(".modal-wrapper .modal.tickets-views .ticket-view .go-send").click(
    function () {
  ...
  function checkCodeVerification(code, email, ticket_id, type, page) {
    if (page != 2) {
      ...
      let cheat_id = $(".modal-wrapper .modal.accept-email input.custom").data(
          "cheat-id"
        ),
        name = $(".modal-wrapper .modal.accept-email input.custom").data(
          "name"
        ),
        email = $(".modal-wrapper .modal.accept-email input.custom").data(
          "email"
        ),
        text = $(".modal-wrapper .modal.accept-email input.custom").data(
          "text"
        ),
        rate = $(".modal-wrapper .modal.accept-email input.custom").data(
          "rate"
        ),
        data_review = [];
